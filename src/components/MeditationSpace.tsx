...
const MeditationSpace = () => {
  ...
  return <>
      <MeditationScene currentScene={currentScene} isTransitioning={isTransitioning} />
      <div className="relative z-10 container mx-auto px-4 py-8">
        <div className="grid md:grid-cols-2 gap-8">
          <div className="bg-white/10 backdrop-blur-lg rounded-2xl p-6">
            <AudioPlayer isPlaying={isPlaying} setIsPlaying={setIsPlaying} currentScene={currentScene} sounds={scenes[currentScene].sounds} />
            <div className="mt-8">
              <h2 className="text-white text-xl mb-4">Duration</h2>
              <div className="flex gap-4">
                {[5, 10, 20].map(duration => <button key={duration} onClick={() => handleDurationSelect(duration)} className={`px-6 py-2 rounded-full ${selectedDuration === duration ? "bg-white text-purple-900" : "bg-white/20 text-white"}`}>
                    {duration} min
                  </button>)}
              </div>
              {timeRemaining !== null && <p className="text-white mt-4">
